// index.js (FULL FILE) — Xbox alt-scrubber + Trader price commands w/ autocomplete (JSON-backed)
...
// ===== Trader JSON path =====
// Put file in volume as /app/data/trader_prices.json (default path resolves to ./data/trader_prices.json)
const TRADER_PRICE_JSON_PATH = (process.env.TRADER_PRICE_JSON_PATH ?? "").trim();
...
console.log(`TRADER_PRICE_JSON_PATH=${TRADER_PRICE_JSON_PATH || "(default ./data/trader_prices.json)"}`);
...
function findBestItemMatch(query, traderIndex) {
  const q = normalizeItemKey(query);
  if (!q) return null;

  if (traderIndex.itemsByKey.has(q)) return traderIndex.itemsByKey.get(q);

  // contains fallback
  for (const it of traderIndex.itemsByKey.values()) {
    if (it?.name?.toLowerCase?.().includes(q)) return it;
  }
  return null;
}
function searchItems(query, traderIndex, limit = 25) {
  const q = normalizeItemKey(query);
  if (!q) return [];
  const out = [];
  for (const it of traderIndex.itemsByKey.values()) {
...
const trader = {
  loaded: false,
  error: "",
  itemsByKey: new Map(), // key(lower item name) -> {name, category, buy, sell}
...
function initTrader() {
  try {
    const fallbackPath = path.resolve(DATA_DIR, "trader_prices.json");
    const jsonPath = TRADER_PRICE_JSON_PATH || fallbackPath;

    if (!fs.existsSync(jsonPath)) {
      trader.loaded = false;
      trader.error = `Trader JSON not found at ${jsonPath}. Put it there or set TRADER_PRICE_JSON_PATH.`;
      console.log("[TRADER]", trader.error);
      return;
    }
    ...
    const itemsObj = parsed?.items_by_key;
    const catsObj = parsed?.items_by_category;
    const catList = parsed?.categories;

    if (!itemsObj || typeof itemsObj !== "object") throw new Error("Invalid trader JSON: missing items_by_key");
    if (!catsObj || typeof catsObj !== "object") throw new Error("Invalid trader JSON: missing items_by_category");
    if (!Array.isArray(catList)) throw new Error("Invalid trader JSON: missing categories array");
    ...
    for (const [k, v] of Object.entries(itemsObj)) {
      ...
      const buy = v.buy === null || v.buy === undefined ? null : Number(v.buy);
      const sell = v.sell === null || v.sell === undefined ? null : Number(v.sell);

      itemsByKey.set(String(k).toLowerCase().trim(), {
        name,
        category: String(v.category ?? "Uncategorized").trim() || "Uncategorized",
    ...
    for (const [cat, arr] of Object.entries(catsObj)) {
      if (!cat) continue;
      if (!Array.isArray(arr)) continue;

      const cleanCat = String(cat).trim() || "Uncategorized";
      const list = arr
        .map((v) => {
          const name = String(v?.name ?? "").trim();
          const buy = v?.buy === null || v?.buy === undefined ? null : Number(v.buy);
          ...
          return {
            name,
            category: String(v?.category ?? cleanCat).trim() || cleanCat,
            buy: Number.isFinite(buy) ? buy : null,
            sell: Number.isFinite(sell) ? sell : null,
          };
        })
      ...
      categories.set(cleanCat, list);
    }

    trader.itemsByKey = itemsByKey;
    trader.categories = categories;
    trader.categoryList = catList.map((c) => String(c).trim()).filter(Boolean);

    trader.loaded = true;
    trader.error = "";

    console.log(
      `[TRADER] Loaded ${trader.itemsByKey.size} items across ${trader.categoryList.length} categories from ${jsonPath}`
    );
  } catch (e) {
    trader.loaded = false;
    trader.error = e?.message ?? String(e);
    console.log("[TRADER] Failed to load JSON:", trader.error);
  }
}
initTrader();
...
// ============================================================================
// ✅ AUTOCOMPLETE (must be AFTER const client = new Client(...))
// ============================================================================
client.on("interactionCreate", async (interaction) => {
  ...
  try {
    ...
    if (!trader.loaded) {
      await interaction.respond([{ name: "Trader list not loaded (missing trader_prices.json)", value: "Trader list not loaded" }].slice(0, 25));
      return;
    }
    ...
    if (cmd === "pricecategory" && focused.name === "category") {
      ...
      const fallback = cats.slice(0, 25).map((c) => ({ name: c, value: c }));
      await interaction.respond(matches.length ? matches : fallback);
      return;
    }

    // Item dropdown for /price /buy /sell and item-like dropdown for /pricesearch
    if (["price", "buy", "sell", "pricesearch"].includes(cmd) && (focused.name === "item" || focused.name === "query")) {
      ...
      for (const it of trader.itemsByKey.values()) {
        if (!it?.name) continue;
        if (!it.name.toLowerCase().includes(typed)) continue;

        const label = it.category ? `${it.name} • ${it.category}` : it.name;
        results.push({ name: label.slice(0, 100), value: it.name });
...
client.on("interactionCreate", async (interaction) => {
  ...
  const cmd = interaction.commandName;
  const isTraderCmd = ["price", "buy", "sell", "pricesearch", "pricecategory"].includes(cmd);
  const isXboxCmd = ["xcheck", "xinfo", "xflagged", "xtrust"].includes(cmd);
  if (!isTraderCmd && !isXboxCmd) return;
  ...
  try {
    ...
    if (isTraderCmd) {
      if (!trader.loaded) {
        await interaction.editReply(`Trader list is not loaded.\n${trader.error || "Missing trader_prices.json."}`);
        return;
      }

      if (cmd === "price") {
        const itemName = interaction.options.getString("item", true);
        const it = findBestItemMatch(itemName, trader);
        if (!it) {
          await interaction.editReply(`No match found for: **${itemName}**`);
          return;
        }

        const embed = new EmbedBuilder()
          .setTitle(`Price • ${it.name}`)
          .setColor(0x2b2d31)
          .addFields(
            { name: "Category", value: it.category || "—", inline: true },
            { name: "Buy (from trader)", value: it.buy !== null ? moneyFmt(it.buy) : "—", inline: true },
            { name: "Sell (to trader)", value: it.sell !== null ? moneyFmt(it.sell) : "—", inline: true }
          )
          .setFooter({ text: "Note: Some items intentionally have no sell price." })
          .setTimestamp();
      ...
      if (cmd === "buy") {
        ...
        if (it.buy === null) {
          await interaction.editReply(`**${it.name}** does not have a buy price listed.`);
          return;
        }

        const total = it.buy * qty;
        const embed = new EmbedBuilder()
          .setTitle(`Buy • ${it.name}`)
          .setColor(0x00ff00)
          .addFields(
            { name: "Category", value: it.category || "—", inline: true },
            { name: "Unit Price", value: moneyFmt(it.buy), inline: true },
            { name: "Qty", value: String(qty), inline: true },
            { name: "Total Cost", value: moneyFmt(total), inline: false }
          )
          .setTimestamp();
      ...
      if (cmd === "sell") {
        ...
        if (it.sell === null) {
          await interaction.editReply(`**${it.name}** does not have a sell price listed.`);
          return;
        }

        const total = it.sell * qty;
        const embed = new EmbedBuilder()
          .setTitle(`Sell • ${it.name}`)
          .setColor(0xffcc00)
          .addFields(
            { name: "Category", value: it.category || "—", inline: true },
            { name: "Unit Price", value: moneyFmt(it.sell), inline: true },
            { name: "Qty", value: String(qty), inline: true },
            { name: "Total Payout", value: moneyFmt(total), inline: false }
          )
          .setFooter({ text: "Note: Some items intentionally have no sell price." })
          .setTimestamp();
      ...
      if (cmd === "pricesearch") {
        ...
        const matches = searchItems(q, trader, 25);

        if (matches.length === 0) {
          await interaction.editReply(`No items found matching: **${q}**`);
          return;
        }

        const lines = matches.map((it) => {
          const buy = it.buy !== null ? moneyFmt(it.buy) : "—";
          const sell = it.sell !== null ? moneyFmt(it.sell) : "—";
          return `**${it.name}** • Buy: ${buy} • Sell: ${sell} • _${it.category}_`;
        });

        const embeds = buildListEmbeds(`Price Search • "${q}" • ${matches.length} result(s)`, lines, 0x2b2d31);
      ...
      if (cmd === "pricecategory") {
        ...
        let items = trader.categories.get(cat) || null;
        if (!items) {
          const lower = cat.toLowerCase();
          const foundKey = trader.categoryList.find((c) => c.toLowerCase() === lower);
          if (foundKey) items = trader.categories.get(foundKey);
        }

        if (!items || items.length === 0) {
          await interaction.editReply(`No items found for category: **${cat}**`);
          return;
        }
        ...
        const lines = items.map((it) => {
          const buy = it.buy !== null ? moneyFmt(it.buy) : "—";
          const sell = it.sell !== null ? moneyFmt(it.sell) : "—";
          return `**${it.name}** • Buy: ${buy} • Sell: ${sell}`;
        });

        const embeds = buildListEmbeds(`Category • ${cat} • ${items.length} item(s)`, lines, 0x2b2d31);
        await interaction.editReply({ embeds: [embeds[0]] });
        for (let i = 1; i < embeds.length; i++) await interaction.followUp({ embeds: [embeds[i]] });
        return;
      }

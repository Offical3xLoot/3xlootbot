...
// ===== Trader price list =====
const TRADER_PRICE_JSON_PATH = (process.env.TRADER_PRICE_JSON_PATH ?? "data/trader_prices.json").trim();
...
// Parse tradecalc items: "3 four dials,1 bottled suppressor, 2 vs89 mags"
function parseTradeItems(input) {
  ...
  return raw
    ...
    .filter(Boolean)
    .map((chunk) => {
      // quantity optional; if not given assume 1
      // accepts: "3 item name", "3x item name", "3× item name"
      const m = chunk.match(/^\s*(\d+)\s*(?:x|×)?\s*(.+?)\s*$/i);
      if (m) {
        const qty = Number.parseInt(m[1], 10);
        const name = (m[2] ?? "").trim();
        return { qty: Number.isFinite(qty) && qty > 0 ? qty : 1, name };
      }
...
// =====================================================
// Trader pricelist loading + index
// =====================================================
let TRADER = {
  loaded: false,
  version: 0,
  categories: [],
  itemsByKey: new Map(), // key -> item
  itemsByCategory: new Map(), // category -> array
};
...
function loadTraderJson() {
  try {
    ...
    if (!fs.existsSync(p)) {
      console.log(`[TRADER] Trader JSON not found at ${TRADER_PRICE_JSON_PATH}. Put it there or set TRADER_PRICE_JSON_PATH.`);
      TRADER.loaded = false;
      return;
    }
    ...
    const itemsByKeyObj = parsed?.items_by_key && typeof parsed.items_by_key === "object" ? parsed.items_by_key : {};
    const itemsByCategoryObj =
      parsed?.items_by_category && typeof parsed.items_by_category === "object" ? parsed.items_by_category : {};

    const itemsByKey = new Map();
    for (const [k, v] of Object.entries(itemsByKeyObj)) {
      if (!k || !v) continue;
      itemsByKey.set(k, v);
    }

    const itemsByCategory = new Map();
    for (const [cat, arr] of Object.entries(itemsByCategoryObj)) {
    ...
    console.log(`[TRADER] Loaded categories=${TRADER.categories.length} items=${TRADER.itemsByKey.size}`);
  } catch (e) {
    console.error("[TRADER] Failed to load trader JSON:", e?.message ?? e);
    TRADER.loaded = false;
  }
}
loadTraderJson();
...
function findTraderItem(query) {
  ...
  for (const it of TRADER.itemsByKey.values()) {
    if (normalizeTextKey(it?.item) === q) return it;
  }

  // contains match
  const hits = [];
  for (const it of TRADER.itemsByKey.values()) {
    const nameKey = normalizeTextKey(it?.item);
    if (!nameKey) continue;
    if (nameKey.includes(q) || q.includes(nameKey)) hits.push(it);
  }
...
function searchTraderItems(query, limit = 50) {
  ...
  const scored = [];
  for (const it of TRADER.itemsByKey.values()) {
    const key = normalizeTextKey(it?.item);
    if (!key) continue;
    ...
    scored.push({ it, score, len: key.length });
  }

  scored.sort((a, b) => b.score - a.score || a.len - b.len || a.it.item.localeCompare(b.it.item));
  return scored.slice(0, limit).map((x) => x.it);
}

function buildPriceEmbed(item, opts = {}) {
  const title = opts.title ?? "Trader Price";
  const e = new EmbedBuilder().setTitle(title).setColor(0x2b2d31).setTimestamp();

  const name = item?.item ?? "Unknown Item";
  const cat = item?.category ?? "Uncategorized";
  const buy = item?.buy ?? null;
  const sell = item?.sell ?? null;
  ...
  const lines = [];
  lines.push(`🟢 **Buy:** $${formatMoney(buy)}`);
  lines.push(`🔴 **Sell:** $${formatMoney(sell)}`);
...
async function autoDeployCommandsIfEnabled() {
  ...
  const commands = [
    ...
    new SlashCommandBuilder()
      .setName("price")
      .setDescription("Open trader price menus (category + item dropdown)."),

    new SlashCommandBuilder()
      .setName("pricesearch")
      .setDescription("Search the trader list by name.")
      .addStringOption((opt) => opt.setName("query").setDescription("Search text").setRequired(true)),

    new SlashCommandBuilder()
      .setName("pricecategory")
      .setDescription("Browse a trader category.")
      .addStringOption((opt) => opt.setName("category").setDescription("Category name").setRequired(true)),

    new SlashCommandBuilder()
      .setName("tradecalc")
      .setDescription("Calculator for bulk buys/sells. Use commas between items.")
      .addStringOption((opt) =>
        opt
          .setName("mode")
          .setDescription("buy = cost, sell = payout, net = sell minus buy")
          .addChoices({ name: "buy", value: "buy" }, { name: "sell", value: "sell" }, { name: "net", value: "net" })
          .setRequired(true)
      )
      .addStringOption((opt) =>
        opt
          .setName("items")
          .setDescription('Example: "3 four dials,1 bottled suppressor,2 vs89 mags"')
          .setRequired(true)
      ),
  ].map((c) => c.toJSON());
...
async function fetchOpenXblMergedProfile(gamertag) {
  ...
  const gamerscore =
    parseIntOrNull(readSetting(settingsMap, "Gamerscore")) ?? parseIntOrNull(person.gamerscore) ?? null;
...
// Build paged embeds (simple)
function buildListEmbeds(title, lines, color = 0x2b2d31) {
  const chunks = chunkLines(lines, 3500);
  const embeds = [];
  for (let i = 0; i < chunks.length; i++) {
    const e = new EmbedBuilder().setTitle(title).setDescription(chunks[i] || "—").setColor(color).setTimestamp();
...
// =====================================================
// Trader UI sessions (select menus + next/prev buttons)
// =====================================================
const UI_SESSIONS = new Map(); // sessionId -> { userId, type, createdMs, data... }
const UI_TTL_MS = 10 * 60 * 1000;

function purgeSessions() {
  const now = nowMs();
  for (const [sid, s] of UI_SESSIONS.entries()) {
    if (!s || now - (s.createdMs ?? 0) > UI_TTL_MS) UI_SESSIONS.delete(sid);
  }
}
setInterval(purgeSessions, 60 * 1000).unref?.();

function makePagerRow(sessionId, page, pageCount, prefix) {
  const prev = new ButtonBuilder()
    .setCustomId(`${prefix}:prev:${sessionId}:${page}`)
    .setLabel("Prev")
    .setStyle(ButtonStyle.Secondary)
    .setDisabled(page <= 0);

  const next = new ButtonBuilder()
    .setCustomId(`${prefix}:next:${sessionId}:${page}`)
    .setLabel("Next")
    .setStyle(ButtonStyle.Secondary)
...
function paginateArray(arr, page, perPage) {
  ...
  return { page: p, pageCount, slice: arr.slice(start, end), total };
}

function buildSearchResultsEmbed(query, items, page, pageCount) {
  const e = new EmbedBuilder()
    .setTitle(`Trader Search: ${query}`)
    .setColor(0x2b2d31)
    .setTimestamp()
    .setDescription(
      items.length
        ? items
            .map((it) => `• **${it.item}** — ${it.category}  \n  Buy: $${formatMoney(it.buy)} • Sell: $${formatMoney(it.sell)}`)
            .join("\n")
        : "No matches."
    )
...
function buildTradeCalcEmbed(mode, parsed, lines, totals) {
  const e = new EmbedBuilder().setTitle("Trader Calculator").setColor(0x2b2d31).setTimestamp();

  e.setDescription(
    [
      `Mode: **${mode.toUpperCase()}**`,
      `Items parsed: **${parsed.length}** (comma-separated)`,
      "",
  ...
  const fields = [];
  if (totals.buy !== null) fields.push({ name: "Buy Total", value: `$${formatMoney(totals.buy)}`, inline: true });
  if (totals.sell !== null) fields.push({ name: "Sell Total", value: `$${formatMoney(totals.sell)}`, inline: true });
  if (totals.net !== null) fields.push({ name: "Net", value: `$${formatMoney(totals.net)}`, inline: true });
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.isButton()) {
    try {
      ...
      // prefixes: psearch / pcat
      const prefix = parts[0];
      const dir = parts[1];
      const sessionId = parts[2];
      const oldPage = Number.parseInt(parts[3] ?? "0", 10) || 0;

      const s = UI_SESSIONS.get(sessionId);
      if (!s) {
        await interaction.reply({ content: "That menu expired. Run the command again.", ephemeral: true });
        return;
      }
      if (s.userId !== interaction.user.id) {
        await interaction.reply({ content: "That menu isn’t yours.", ephemeral: true });
        return;
      }
      ...
      if (prefix === "psearch" && s.type === "search") {
        ...
        const embed = buildSearchResultsEmbed(s.query, slice, p, pageCount);
        const row = makePagerRow(sessionId, p, pageCount, "psearch");
      ...
      if (prefix === "pcat" && s.type === "category") {
        ...
        const embed = buildCategoryEmbed(s.category, slice, p, pageCount);
        const row = makePagerRow(sessionId, p, pageCount, "pcat");
        await interaction.update({ embeds: [embed], components: [row] });
        return;
      }
      ...
      try {
        if (interaction.deferred || interaction.replied) await interaction.editReply("Button error.");
        else await interaction.reply({ content: "Button error.", ephemeral: true });
      } catch {}
    }
    return;
  }
  ...
  if (interaction.isStringSelectMenu()) {
    try {
      ...
      if (prefix === "pricecat" && s.type === "price") {
        ...
        const options = items.slice(0, 25).map((it) => ({
          label: it.item.slice(0, 100),
          value: it.key ?? normalizeTextKey(it.item),
          description: `Buy $${formatMoney(it.buy)} • Sell $${formatMoney(it.sell)}`.slice(0, 100),
        }));

        const itemSelect = new StringSelectMenuBuilder()
          .setCustomId(`priceitem:${sessionId}`)
          .setPlaceholder("Select an item…")
          .addOptions(options);

        const rows = [new ActionRowBuilder().addComponents(itemSelect)];

        const embed = new EmbedBuilder()
          .setTitle("Trader Price")
          .setColor(0x2b2d31)
          .setTimestamp()
          .setDescription(`Category selected: **${category}**\nNow pick an item from the dropdown.`);

        await interaction.update({ embeds: [embed], components: rows });
        return;
      }

      if (prefix === "priceitem" && s.type === "price") {
        ...
        // Item objects in JSON should include a stable "key" but we fall back to name key
        const item =
          TRADER.itemsByKey.get(key) ||
          TRADER.itemsByKey.get(normalizeTextKey(key)) ||
          findTraderItem(key);

        if (!item) {
          await interaction.reply({ content: "Couldn’t find that item in the trader list.", ephemeral: true });
          return;
        }

        const embed = buildPriceEmbed(item, { title: "Trader Price" });
      ...
      try {
        if (interaction.deferred || interaction.replied) await interaction.editReply("Menu error.");
        else await interaction.reply({ content: "Menu error.", ephemeral: true });
      } catch {}
    }
    return;
  }
  ...
  try {
    ...
    // Trader commands require pricelist
    if (["price", "pricesearch", "pricecategory", "tradecalc"].includes(cmd) && !TRADER.loaded) {
      await interaction.reply({
        content: "Trader list not loaded. Make sure trader_prices.json exists and TRADER_PRICE_JSON_PATH points to it.",
        ephemeral: true,
      });
      return;
    }
    ...
    // =====================================================
    if (cmd === "price") {
      const categories = TRADER.categories.slice().sort((a, b) => a.localeCompare(b));
      ...
      UI_SESSIONS.set(sessionId, { userId: interaction.user.id, type: "price", createdMs: nowMs() });

      const select = new StringSelectMenuBuilder()
        .setCustomId(`pricecat:${sessionId}`)
      ...
      const embed = new EmbedBuilder()
        .setTitle("Trader Price Lookup")
        .setColor(0x2b2d31)
        .setTimestamp()
        .setDescription("Pick a **category** from the dropdown to continue.");
    ...
    if (cmd === "pricesearch") {
      ...
      const embed = buildSearchResultsEmbed(query, slice, page, pageCount);
      const row = makePagerRow(sessionId, page, pageCount, "psearch");
    ...
    if (cmd === "pricecategory") {
      ...
      const categoryKey = TRADER.categories.find((c) => normalizeTextKey(c) === normalizeTextKey(categoryInput));
      const category = categoryKey ?? categoryInput;
      ...
      const embed = buildCategoryEmbed(category, slice, page, pageCount);
      const row = makePagerRow(sessionId, page, pageCount, "pcat");
    ...
    if (cmd === "tradecalc") {
      ...
      for (const ent of parsed) {
        ...
        if (buy !== null) {
          hadBuy = true;
          buyTotal += Number(buy) * ent.qty;
        }
        if (sell !== null) {
          hadSell = true;
          sellTotal += Number(sell) * ent.qty;
        }
      }

      const totals = {
        buy: hadBuy ? buyTotal : null,
        sell: hadSell ? sellTotal : null,
        net: hadBuy || hadSell ? (hadSell ? sellTotal : 0) - (hadBuy ? buyTotal : 0) : null,
      };

      // Apply mode to show relevant totals (still show all if present)
      const embed = buildTradeCalcEmbed(mode, parsed, lines.slice(0, 30), totals);

      if (missing.length) {
        embed.addFields({
          name: "Not Found",
          value: missing.slice(0, 15).map((x) => `• ${x}`).join("\n"),
          inline: false,
        });
      }
    ...
    // =====================================================
    // XBL commands (original functionality)
    // =====================================================
    if (!["xcheck", "xinfo", "xflagged", "xtrust"].includes(cmd)) {
    ...
    if (cmd === "xtrust") {
      ...
      if (action === "add") {
        ...
        if (addedList.length)
          embed.addFields({ name: "Added Gamertags", value: addedList.join("\n").slice(0, 1024), inline: false });
        if (alreadyList.length)
          embed.addFields({ name: "Already Trusted", value: alreadyList.join("\n").slice(0, 1024), inline: false });
        if (invalidList.length)
          embed.addFields({ name: "Invalid", value: invalidList.join("\n").slice(0, 1024), inline: false });
      ...
      if (action === "remove") {
        ...
        if (removedList.length)
          embed.addFields({ name: "Removed Gamertags", value: removedList.join("\n").slice(0, 1024), inline: false });
        if (notFoundList.length)
          embed.addFields({ name: "Not Trusted", value: notFoundList.join("\n").slice(0, 1024), inline: false });
    ...
    if (cmd === "xflagged") {
      ...
      const embeds = buildListEmbeds(`Flagged (All-Time) • ${lines.length}`, lines.length ? lines : ["No saved low-GS gamertags yet."], 0xff0000);
